...
describe('rover', function () {
    ...
    it('init rover', function () {
        const rover = new Rover(new Coordinates(3, 4, Direction.SOUTH), new WaterDetector(), new DeepNetworkSpaceReportSender());
    ...
    it('should advance north', () => {
        const rover = new Rover(new Coordinates(3, 4, Direction.NORTH), new WaterDetector(), new DeepNetworkSpaceReportSender());
        expect(rover.advance().coordinates).toStrictEqual(new Coordinates(3, 5, Direction.NORTH))
    });


    it('should advance east', () => {
        const rover = new Rover(new Coordinates(3, 4, Direction.EAST), new WaterDetector(), new DeepNetworkSpaceReportSender());
        expect(rover.advance().coordinates).toStrictEqual(new Coordinates(2, 4, Direction.EAST))
    });

    it('should advance west', () => {
        const rover = new Rover(new Coordinates(3, 4, Direction.WEST), new WaterDetector(), new DeepNetworkSpaceReportSender());
    ...
    `('rotate from $init to "$expected"', ({init, expected}) => {
        const rover = new Rover(new Coordinates(3, 4, init), new WaterDetector(), new DeepNetworkSpaceReportSender());
    ...
    `('could turn around mars', ({init, expected}) => {
        const rover = new Rover(init, new WaterDetector(), new DeepNetworkSpaceReportSender());
